      ...
      <div className="max-w-7xl mx-auto">
            ...
            return (
              <motion.div
                ...
                animate={isHighlighted ? {
                  ...
                  // Meredupkan kartu lain saat ada yang aktif
                ...
                transition={{ 
                  ...
                  // Highlight menggunakan durasi 5 detik
                  default: { duration: isHighlighted ? 5 : 0.4, ease: "easeInOut" },
                  // Transisi snappy untuk interaksi hover
              ...
              >
                ...
                <motion.div layoutId={`image-wrapper-${item.id}`} className="absolute inset-0 z-0">
                  <img src={item.image} alt={item.title} className="w-full h-full object-cover object-top opacity-70 group-hover:opacity-100 transition-all duration-500" />
                </motion.div>
        ...
        <AnimatePresence>
          {selectedId && selectedItem && (
            ...
            >
              <motion.div layoutId={`card-container-${selectedId}`} className="relative w-full h-full max-w-6xl max-h-[90vh] flex items-center justify-center rounded-3xl overflow-hidden" onClick={(e) => e.stopPropagation()}>
                 <motion.div layoutId={`image-wrapper-${selectedId}`} className="relative w-full h-full flex items-center justify-center">
                    <img src={selectedItem.image} alt={selectedItem.title} className="w-auto h-auto max-w-full max-h-full object-contain shadow-2xl" />
                </motion.div>
                <button onClick={() => setSelectedId(null)} className="absolute top-4 right-4 bg-black/50 text-white p-2 rounded-full hover:bg-emerald-500 transition-colors">
